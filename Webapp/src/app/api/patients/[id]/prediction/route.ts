...
function formatHealthDataForAnalysis(vitals: VitalData[], breathData: BreathData[]): string {
  ...
  if (vitals.length === 0) {
    ...
    vitals.forEach((vital, index) => {
      ...
      if (vital.heart_rate_bpm) dataText += `- Heart Rate: ${vital.heart_rate_bpm} bpm\n`;
      if (vital.pulse_bpm) dataText += `- Pulse: ${vital.pulse_bpm} bpm\n`;
      if (vital.spo2_percent) dataText += `- SpO2: ${vital.spo2_percent}%\n`;
      if (vital.body_temp_c) dataText += `- Body Temperature: ${vital.body_temp_c}°C\n`;
  ...
  if (breathData.length === 0) {
    ...
    breathData.forEach((breath, index) => {
      ...
      if (breath.ammonia_ppm) dataText += `- Ammonia: ${breath.ammonia_ppm} ppm\n`;
      if (breath.co2_ppm_mq) dataText += `- CO2 (MQ): ${breath.co2_ppm_mq} ppm\n`;
      if (breath.co2_ppm_mhz19) dataText += `- CO2 (MHZ19): ${breath.co2_ppm_mhz19} ppm\n`;
      if (breath.benzene_ppm) dataText += `- Benzene: ${breath.benzene_ppm} ppm\n`;
      if (breath.ethanol_ppm) dataText += `- Ethanol: ${breath.ethanol_ppm} ppm\n`;
      if (breath.vocs_ppm_mics) dataText += `- VOCs (MICS): ${breath.vocs_ppm_mics} ppm\n`;
      if (breath.acetone_ppm_qcm) dataText += `- Acetone (QCM): ${breath.acetone_ppm_qcm} ppm\n`;
...
export async function GET(
  ...
  try {
    ...
    if (cacheEntry) {
      ...
      const latestVitalTime = latestVital ? new Date(latestVital.recorded_at).getTime() : 0;
      const latestBreathTime = latestBreath ? new Date(latestBreath.recorded_at).getTime() : 0;
    ...
    const formattedData = formatHealthDataForAnalysis(vitalsData || [], breathData || []);
